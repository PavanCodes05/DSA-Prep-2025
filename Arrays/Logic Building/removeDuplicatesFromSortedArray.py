...
Approach:
    ...
    In last problem we had to move all the zeros to the end, now we have to move all the
    duplicates to somewhere, we doesn't have to explicitly move it to the end but it is possible,
    We use two pointers, 'i' being with the first element, because we know that element is already perfect
    if we have only one element that is going to be perfect, so we keep the other pointer on the next element
    and we move the right pointer till we find a non-equal value of the first pointer, once we find that we move the 'i' pointer and assign the non-equal value to the current 'i' position, now we have the perfect
    values alligned in the same order but avoiding duplicates, now we can finally return the 'i + 1' value, because the final value inserted in the i-th position will be the number of unique elements and as i starts 
    from 0 we have to add 1 to it and return it.
